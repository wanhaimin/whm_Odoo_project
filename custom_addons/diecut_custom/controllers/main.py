...
class MaterialWebsite(http.Controller):
    ...
    def materials_list(self, page=1, category=None, search=None, **kwargs):
        ...
        # 分类筛选
        if category:
            category_obj = request.env['my.material.category'].sudo().browse(int(category))
        ...
        # 获取所有分类
        categories = request.env['my.material.category'].sudo().search([])
    ...
    def sample_order_submit(self, **post):
        ...
        order_vals = {
            ...
            'quantity': int(post.get('quantity', 10)),
        ...
        line_vals = {
            ...
            'length': float(post.get('length')),
            'width': float(post.get('width')),
            'thickness': float(post.get('thickness', 0)),
            'quantity': int(post.get('quantity', 10)),
