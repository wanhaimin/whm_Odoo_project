...
class CatalogActivateWizard(models.TransientModel):
    ...
    # --- 来源信息（自动带入，只读） ---
    variant_id = fields.Many2one('product.product', string='源变体', required=True, readonly=True)
    catalog_tmpl_id = fields.Many2one('product.template', string='源选型目录', required=True, readonly=True)
    ...
    def default_get(self, fields_list):
        ...
        catalog_tmpl_id = res.get('catalog_tmpl_id') or self.env.context.get('default_catalog_tmpl_id')
    ...
    def action_confirm(self):
        ...
        if variant.variant_color:
            color = self.env['diecut.color'].search([('name', 'ilike', variant.variant_color.strip())], limit=1)
            if not color:
                color = self.env['diecut.color'].create({'name': variant.variant_color.strip()})
