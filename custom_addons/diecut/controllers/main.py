...
class CatalogCsvGridController(http.Controller):
    _ALLOWED = {"catalog_items.csv", "catalog_item_specs.csv", "catalog_spec_defs.csv", "catalog_series.csv"}
...
<div class="toolbar"><strong>AG Grid CSV编辑器</strong><span>文件: {safe_name}</span><button id="reloadBtn">刷新</button><button id="addRowBtn">新增行</button><button id="saveBtn" class="primary">保存</button><span id="status" class="status">加载中...</span></div>
...
document.getElementById('saveBtn').addEventListener('click',saveData);loadData();
    ...
    def csv_grid_data(self, file="catalog_items.csv", **kwargs):
        ...
        return request.make_response(json.dumps({"ok": True, "headers": headers, "rows": rows}, ensure_ascii=False), headers=[("Content-Type", "application/json; charset=utf-8")])
    ...
    def _validate_csv_primary_keys(self, safe_name, headers, rows):
        ...
        if safe_name == "catalog_items.csv":
            ...
            needs_brand = True
        elif safe_name == "catalog_spec_defs.csv":
        ...
        for idx, row in enumerate(rows):
            ...
            if needs_brand:
                ...
                if not brand_key:
                    errors.append({"rowIndex": idx, "field": "brand_id_xml/brand_name", "message": "不能同时为空"})
                parts.append(brand_key)
            for f in key_fields:
                v = (row.get(f) or "").strip().lower()
                if not v:
                    errors.append({"rowIndex": idx, "field": f, "message": "不能为空"})
                parts.append(v)
            if all(parts):
                key = tuple(parts)
                if key in seen:
    ...
    def csv_grid_save(self, **kwargs):
        ...
        if validation_errors:
            return request.make_response(json.dumps({"ok": False, "error": "主键校验未通过", "errors": validation_errors}, ensure_ascii=False), headers=[("Content-Type", "application/json; charset=utf-8")])
        output = io.StringIO()
        ...
        return request.make_response(json.dumps({"ok": True, "row_count": len(rows)}, ensure_ascii=False), headers=[("Content-Type", "application/json; charset=utf-8")])
